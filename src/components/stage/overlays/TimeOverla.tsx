...
export const TimeOverlay = () => {
    ...
    const timelineImages = useMemo(() => {
        ...
        return (currentImages ?? [])
            .map((image) => {
                ...
                const rawPercent = ((ms - timeBounds.from.getTime()) / timeBounds.spanMs) * 100;
                const percent = Math.max(0, Math.min(100, Number(rawPercent.toFixed(2))));
                return { id: image.id, ms, percent, label: new Date(ms).toLocaleTimeString() };
            })
    ...
    useEffect(() => {
        const id = window.setTimeout(() => setDebouncedSliderValue(sliderValue), DEBOUNCE_MS);
    ...
    useEffect(() => {
        if (!timeBounds) return;
        const fromTime = timeBounds.from.getTime() + (timeBounds.spanMs * debouncedSliderValue[0]) / 100;
        const toTime = timeBounds.from.getTime() + (timeBounds.spanMs * debouncedSliderValue[1]) / 100;
        useTimeStore.getState().setInterval(new Date(fromTime), new Date(Math.max(fromTime, toTime)));
                                    ...
                                    {timelineImages.map((tick) => (
                                        <Popover key={tick.id}>
                                            ...
                                            <PopoverContent className="w-64" side="top" align="center">
                                                <PopoverHeader>
                                                    <PopoverTitle className="text-sm font-bold">Image {tick.id}</PopoverTitle>
                                                    <PopoverDescription className="text-xs">
                                                        {new Date(tick.ms).toLocaleString()}
                                                    </PopoverDescription>
                                                </PopoverHeader>
                                                <div className="mt-3 flex justify-end">
                                                    <Button size="sm" onClick={() => zoomToImage(tick.percent, tick.id)}>
                                                        Zoom to image
                                                    </Button>
                                                </div>
                                            </PopoverContent>
