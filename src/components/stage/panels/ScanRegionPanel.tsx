import { scanPatternOptions, useScansStore, type ScanPattern } from '@/store/scansStore';
...
import { ScanRegionArgsSchema, ScanRegionDefinition, useScanRegion } from '@/hooks/generated';
                ...
                <button 
                    ...
                    <div className="flex flex-col gap-1.5">
                        <Select 
                            ...
                            onValueChange={(val) => updateRegion(screenPos.region.id, { pattern: val as ScanPattern })}
                    ...
                    <div className="flex flex-row my-auto gap-2">
                        <Label className="text-[9px] text-slate-400 uppercase tracking-wider">Overlap</Label>
                        <Input 
                            type="number" 
                            min="0" max="99" 
                            className="h-7 text-xs bg-slate-800 border-slate-700 text-white"
                            value={screenPos.region.overlap}
                            onChange={(e) => updateRegion(screenPos.region.id, { overlap: Number(e.target.value) })}
                        />
                    </div>
                     <button 
