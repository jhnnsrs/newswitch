...
const ScanPathPreview = ({ region }: { region: ScanRegion }) => {
    ...
    const { width: pixelWidth, height: pixelHeight } = useCurrentPixelDimensions();

    const { pathPoints, fovWidth, fovHeight } = useMemo(() => {
        if (!affine || !pixelWidth || !pixelHeight) return { pathPoints: [], fovWidth: 0, fovHeight: 0 };

        const scaleX = Math.sqrt(affine[0][0] * affine[0][0] + affine[1][0] * affine[1][0]);
        const scaleY = Math.sqrt(affine[0][1] * affine[0][1] + affine[1][1] * affine[1][1]);
        ...
        return { pathPoints: points, fovWidth: calculatedFovWidth, fovHeight: calculatedFovHeight };
        ...
        <group>
            ...
            {pathPoints.map((pt, i) => (
                <mesh key={`marker-${i}`} position={[pt[0], pt[1], pt[2] + 0.01]}>
                    <circleGeometry args={[Math.max(0.5, pathPoints.length > 100 ? 0.2 : 1), 16]} />
