...
export const PanelProvider = ({ children }: { children: ReactNode }) => {
    const [activePanel, setActivePanel] = useState<PanelContextType['activePanel']>(null);

    const openPanel = (options: { screenPos: { x: number; y: number }; worldPos: Vector3; panelType: RegisteredPanels }) => {
...
export const usePanelOpen = () => {
    ...
    const open = (worldPos: Vector3, panelType: RegisteredPanels) => {
        ...
        console.log('Opening panel at world pos', worldPos, 'which projects to screen pos', { x, y });
