...
function validateSnapshots(
  ...
  return Object.fromEntries(
    Object.entries(snapshotMap).map(([stateKey, revisedState]) => {
      ...
      if (!parsed.success) {
        console.error(
          `[RealtimeProvider] Checkout validation failed for ${appKey}.${stateKey}`,
          {
            error: parsed.error,
...
export function RealtimeProvider({ children }: RealtimeProviderProps) {
  ...
  const cleanupIfAppNotLive = useCallback(
    ...
    [getLiveCount, globalStateStoreRegistry, syncConnectionState],
  );

  const syncAllLiveData = useCallback(
    async (appKey: AppKey) => {
      const availableDefinitions = Object.values(definitions).filter(
        (definition) => definition.appKey === appKey,
      );
      const stateKeys = availableDefinitions.map((definition) => definition.key);
      ...
      if (stateKeys.length > 0) {
        ...
        const { current_global_revision: globalRevision } = response;
        const stateStore = globalStateStoreRegistry.getStoreApi(appKey).getState();
      ...
      const locks = await transport.fetchLocks(appKey);
      lockStoreRegistry.getStoreApi(appKey).getState().replaceLocks(
        Object.fromEntries(
          Object.entries(locks).map(([key, value]) => [key, value.task_id]),
        ),
      );
    },
    [definitions, globalStateStoreRegistry, lockStoreRegistry, transport],
  );
  ...
  const startLive = useCallback(
    async (appKey: AppKey) => {
      ...
      ensureAppLiveSubscription(appKey);
      await syncAllLiveData(appKey);
    },
    [ensureAppLiveSubscription, getLiveCount, globalStateStoreRegistry, syncAllLiveData],
  );

  const stopLiveSync = useCallback(
    async (appKey: AppKey) => {
      const nextCount = Math.max(0, getLiveCount(appKey) - 1);
    ...
    [cleanupIfAppNotLive, getLiveCount],
  );
    ...
    async <T extends Record<string, unknown>, TKey extends string>(
      ...
      const request = transport
        ...
        .then((response) => {
          ...
          if (!parsed.success) {
            console.error(`[RealtimeProvider] Validation failed for ${definition.appKey}.${definition.key}`, {
              error: parsed.error,
  ...
  const stateGoLive = useCallback<StateContextValue['goLive']>(
    async (appKey) => {
  ...
  const stateStopLive = useCallback<StateContextValue['stopLive']>(
    async (appKey) => {
      await stopLiveSync(appKey);
    },
    [stopLiveSync],
  );

  const taskGoLive = useCallback<TaskContextValue['goLive']>(
    async (appKey) => {
      await startLive(appKey);
    },
    [startLive],
  ...
  const taskStopLive = useCallback<TaskContextValue['stopLive']>(
    async (appKey) => {
      await stopLiveSync(appKey);
    },
    [stopLiveSync],
  ...
  const lockGoLive = useCallback<LockContextValue['goLive']>(
    async (appKey) => {
      await startLive(appKey);
    },
    [startLive],
  ...
  const lockStopLive = useCallback<LockContextValue['stopLive']>(
    async (appKey) => {
      await stopLiveSync(appKey);
    },
    [stopLiveSync],
  ...
  const stateValue = useMemo<StateContextValue>(
    () => ({
      ...
      checkout,
      goLive: stateGoLive,
      stopLive: stateStopLive,
    }),
    [checkout, definitions, ensureState, refetchAll, refetchState, stateGoLive, stateStopLive],
  );
  ...
  const lockValue = useMemo<LockContextValue>(
    () => ({
      goLive: lockGoLive,
      stopLive: lockStopLive,
    }),
    [lockGoLive, lockStopLive],
  );
